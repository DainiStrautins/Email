                ...
                if (isset($emailData['attachments'])) {
                    foreach ($emailData['attachments'] as $hash => $attachmentInfo) {
                        if ($attachmentInfo['original_file_name'] === $requestedFileName ||
                            ...
                            $emailInfo = [
                                'emailId' => $id,
                                'originalFileName' => $attachmentInfo['original_file_name'],
                            ];
            ...
            if ($emailInfo) {
                ...
                if (file_exists($emlFilePath)) {
                    ...
                    if (preg_match($pattern, $emlContent, $matches)) {
                        $base64Contents[] = trim($matches[1]);
                    } else {
                        // Handle if base64 content extraction fails
                        $base64Contents[] = 'Base64 content not found in email';
                    }
                } else {
                    // Handle if .eml file doesn't exist
                    $base64Contents[] = 'Email file not found';
                }
            } else {
                // Handle if requested file name not found in processed emails
                $base64Contents[] = 'File not found in processed emails';
            }
        }
